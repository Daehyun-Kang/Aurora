...
def get_tu(source, lang='c', all_warnings=False, flags=[], options=0):
    ...
    if os.path.isfile(source):
        return TranslationUnit.from_source(source, args, options=options)
        #return TranslationUnit.from_source(source, args)
    else:
        return TranslationUnit.from_source(name, args, unsaved_files=[(name, source)], options=options)
