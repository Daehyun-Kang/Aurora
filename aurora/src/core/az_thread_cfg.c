...
az_xml_element_t *az_thread_cfg_getlist(int cfg_cat)
{
...
az_xml_element_t *az_thread_cfg_getfirst(int cfg_cat)
{
  ...
  do {
    parent = az_thread_cfg_getlist(cfg_cat);
...
az_xml_element_t *az_thread_cfg_getnext(int cfg_cat, az_xml_element_t *cur)
{
  az_xml_element_t *parent = NULL;
...
az_r_t az_thread_cfg_convPolicy(az_xml_element_t *attr, void *pVar, int varlen)
{
  az_assert(NULL != attr);
  az_assert(NULL != pVar);
  az_r_t r = AZ_SUCCESS;
...
az_r_t az_thread_cfg_xml2cfg(az_xml_element_t *elem, az_thread_config_t *cfg)
{
